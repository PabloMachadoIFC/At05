    ...
    class Quadro{
        ...
        public function __construct($id, $nome){
            ...
            foreach($quadrados as $quadrado){
                $qA = New Quadrado($quadrado['id'],$quadrado['lado'],$quadrado['cor'],$quadrado['un'],$quadrado['id_quadro']);
                $this->addForma($qA);
            }
            $circulos = Circulo::listar(3,$id);
            foreach($circulos as $circulo){
                $qB = New Circulo($circulo['id'],$circulo['raio'],$circulo['cor'],$circulo['un'],$circulo['id_quadro']);
                $this->addForma($qB);
            }
            $retangulos = Retangulo::listar(3,$id);
            foreach($retangulos as $retangulo){
                $qC = New Retangulo($retangulo['id'],$retangulo['largura'],$retangulo['altura'],$retangulo['cor'],$retangulo['un'],$retangulo['id_quadro']);
                $this->addForma($qC);
            }
            $triangulos = Triangulo::listar(3,$id);
            foreach($triangulos as $triangulo){
                $qD = New Triangulo($triangulo['id'],$triangulo['lado1'],$triangulo['lado2'],$triangulo['lado3'],$triangulo['cor'],$triangulo['un'],$triangulo['tipo'],$triangulo['id_quadro']);
                $this->addForma($qD);
            }
        }
